...
export default function BowerGlimpseSection() {
  ...
  // Image positions and properties
  const images = [
    {
  ...
  return (
    <section className="relative bg-white px-4 md:px-10 lg:px-[60px] py-8 md:py-10 overflow-hidden">
      <div className="max-w-[1440px] mx-auto flex flex-col gap-6 md:gap-8 lg:gap-[34px] items-center">
        ...
        <div className="relative w-full lg:w-[1434px] h-auto lg:h-[735px] rounded-[16px] md:rounded-[20px] lg:rounded-[24px] overflow-hidden">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 lg:hidden">
            {images.slice(0, 6).map((image, index) => (
              <div
                key={index}
                className={`relative aspect-[4/3] rounded-[12px] overflow-hidden transition-all duration-700 ease-out ${
                  isVisible
        ...
        {/* CTA Button */}
        <button
          className="relative flex items-center justify-center gap-2 md:gap-4 h-10 md:h-11 px-4 py-2 rounded-[8px] border border-[#ff8829] backdrop-blur-[32px] group hover:bg-[#ff8829] transition-all duration-300"
          style={{
            background: 'radial-gradient(ellipse at center, rgba(255, 136, 41, 0.1) 0%, rgba(255, 136, 41, 0) 100%)'
          }}
        >
          <span
            className="text-[16px] md:text-[18px] text-[#ff8829] font-medium leading-normal md:leading-[27px] group-hover:text-white transition-colors"
            style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
          >
            Take a campus tour
          </span>
